package com.rodri.practica5_SCB;
/*
...
public class HTTPServer {
	...
	public HTTPServer(File basedir, int port, String indexfich) {
		try {
			...
			for (int i = 0; i < NUM_THREADS; i++) {
				Thread t = new Thread( new HTTPRequest (baseDirectory, indexFileName));
				t.start();
			}
			// Servir
			while (true) {
	...
	public static void main(String[] args) {
		...
		try {
			HTTPServer webserver = new HTTPServer(baseDir,port,nomfich);
		} catch (Exception ex) {
